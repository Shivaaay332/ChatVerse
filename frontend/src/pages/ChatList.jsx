...
import { Search, UserPlus, Loader, MessageSquare, ChevronRight } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24 pt-2">
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 mx-4 rounded-[24px] shadow-sm border border-gray-50 dark:border-gray-700 mt-2 overflow-hidden">
            {recentChats.length > 0 ? recentChats.map((user) => (
              <div key={user.unique_id} onClick={() => navigate(`/chat/${user.unique_id}`, { state: { name: user.username, id: user.unique_id } })} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors border-b border-gray-50 dark:border-gray-700 last:border-0">
